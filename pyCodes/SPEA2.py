...
# # SPEA2
# 
# > This code performs genetic algorithms of the SPEA2 type for the continuous or binary case.
...
def create_Environment_For_SPEA2_Continuous_Case(BOUND_LOW, BOUND_UP, NDIM,
                                                 attr_individual_function, evaluate_function, weights,
                                                 eta = 20.0, indpb=None):
    '''
    Intro:
        This function returns an environment to use
        in the SPEA2_Continuous_Case.
    ---
    Input:
        BOUND_LOW: Float
...
def SPEA2_Continuous_Case(envrironment, NGEN=250, MU=100, CXPB=0.9, seed=None,showprogress = True):
    '''
    Intro:
    ...
    archive = toolbox.select(pop + archive, len(pop + archive))
    ...
    for gen in range(1, NGEN):
        # Vary the population
        mating_pool = toolbox.selectTournament(archive, MU)
...
def intToIndividual(cindividual, bL, n):
    ...
    ---
    Output:
        A individual for a discrete case of SPEA2.
...
def getPop(toolindv, n_database, n_points, random_labels=False, seed=None):
    ...
    Intro:
        ...
        and creates a individual over each label to feed the SPEA2.
    ---
    ...
    ---
    Output:
        A List of individuals for a discrete SPEA2 case to
        create a population
    ''' 
    random.seed(seed)
    pop = []
...
def create_Environment_For_SPEA2_Discrete_Case(X, bss, weights, evaluate_function,
                                               random_labels=False, indpb = None, seed=None):
    '''
    Intro:
        This function returns an environment to use
        in the SPEA2_Discrete_Case.
...
def SPEA2_Discrete_Case(envrironment, NGEN=100, MU=200, CXPB=0.93, seed=None, showprogress = True):
    '''
    Intro:
        This function returns an the best population and some
